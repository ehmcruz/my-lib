...
/*
	Custom STL allocator for PoolManager.
...
public:
	...
	PoolAllocatorSTL (const PoolAllocatorSTL& other)
		: pool_manager(other.pool_manager)
	{
	}

	template <typename Tother>
	PoolAllocatorSTL (const PoolAllocatorSTL<Tother>& other)
		: pool_manager(other.pool_manager)
	{
	}
	...
	[[nodiscard]] T* allocate (const size_type n, const void* = static_cast<const void*>(0))
	{
		...
		return this->pool_manager.allocate_type<T>(n);
	}

	void deallocate (T *p, const size_type n)
	{
		this->pool_manager.deallocate_type<T>(p, n);
	}

	template<typename Tother>
	friend constexpr bool operator== (const PoolAllocatorSTL&, const PoolAllocatorSTL<Tother>&)
	_GLIBCXX_NOTHROW
	{
		return true;
	}
	...
	template <typename Tother>
	friend constexpr bool operator!= (const PoolAllocatorSTL&, const PoolAllocatorSTL<_Up>&)
	_GLIBCXX_NOTHROW
	{
		return false;
	}
